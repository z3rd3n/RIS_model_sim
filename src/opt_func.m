function ret = opt_func(Zvec)
% Serden Sait Eranil
% Ramazan Umut Aktas
SNR = zeros(1,500);
R = zeros(1,500);
for ort = 1:500
global k dis env N Tx_xyz RIS_xyz Rx_xyz Frequency siso P
rng('shuffle');
Z = zeros(N,1);
for nn = 1:N
...
if siso == 1
    ...
    elseif env==2 % WITH NEW CLUSTERS
        ...
        for counter2=indices_3
            ...
            h_SISO_NLOS = h_SISO_NLOS + abs(((randn+1i*randn)/sqrt(2)))*sqrt(Lcs_SISO_3)*transpose(GeT)*alfa_cs_3*GeR;
        end
...
if env == 2
    ...
    for nn = 1:N
        for cs=1:M_new
            for kl = 1:M_new_2
                ...
                H_slack = h_ncs(:,:,nn,cs)*RIS_deneme(theta_RIS(cs),Z(nn))*g_nkl(:,:,nn,kl) ;
                H = H + H_slack;
            end
        end
    end
    ...
    for nn = 1:N
        for cs=1:M_new
            ...
            H_slack = h_ncs(:,:,nn,cs)*RIS_deneme(theta_RIS(cs),Z(nn))*g_n(:,:,nn) ;
            H = H + H_slack;
        end
    end
    H = H + h_SISO;
...
Noise = db2pow(-130);
SNR(ort)=P*-abs(H)^2 / Noise;
R(ort)=log2(1+SNR(ort)); %achievable rate
fprintf(" iteration = " +ort+"\n");
